...
class GoneViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, GonePhotoCollectionViewCellDelegate{
...
//        cell.nameLabel.text = cafeNames[indexPath.row]
        ...
        //ここでハートの画像を変える操作をしたい！！
//        if homeImages[indexPath.row].gone == true {
//            cell.goneButton.setImage(UIImage(named: "heart-fill"), for: .normal)
//        } else {
//            cell.goneButton.setImage(UIImage(named: "heart-outline"), for: .normal)
//        }
...
//        query?.includeKey("user")
        ...
        //付け足したところ
        query?.whereKey("goneUser", equalTo: currentUser.objectId)
...
//                    }
    ...
    //ここでコレクションビューのボタンを押した時の操作がわからない
    func didTapGoneButton(collectionViewCell: UICollectionViewCell, button: UIButton) {
        let alertController = UIAlertController(title: nil, message: nil, preferredStyle: .actionSheet)
        
        let removeAction = UIAlertAction(title: "'行った'から外す", style: .default) { (action) in
            let query = NCMBQuery(className: "HomeImage")
            query?.getObjectInBackground(withId: self.homeImages[0].objectId, block: { (post, error) in
                if error != nil {
                    SVProgressHUD.showError(withStatus: error!.localizedDescription)
                } else {
